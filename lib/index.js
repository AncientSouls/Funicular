...
this._parents={};this.data=undefined}/**
   ...
   * @param error
   */},{key:'mountChilds',value:function mountChilds(callback){var _this2=this;var childCarriages=[];(0,_each2.default)(this.childs,function(child,nextChild){_this2.funicular.mount(child,function(error,carriage){if(error)nextChild(error);else{childCarriages.push(carriage);_this2.tieChild(carriage);nextChild()}})},function(error){if(error){(0,_each2.default)(childCarriages,function(child,nextChild){child.unmount(function(){return nextChild()})},function(){callback(error)})}else callback()})}},{key:'tieChild',value:function tieChild(child){this._childs[child.name]=child;child._parents[this.name]=this}},{key:'untie',value:function untie(){}}]);return Carriage}();/**
 * @example
