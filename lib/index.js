...
this._parents={};this.data=undefined;this.roots=[]}/**
   ...
   */},{key:'mount',value:function mount(mountedCallback){var _this=this;this.subscribe(function(error){if(error)mountedCallback(error);else{_this.generateChildNamesFromData();_this.mountChilds(function(error){if(error){_this.unsubscribe();mountedCallback(error)}else _this.unsafeMount(mountedCallback)})}})}/**
   ...
   * Attention! Not for personal usage.
   * @param {Carriage~unmountCallback} [callback]
   */},{key:'unsafeUnmount',value:function unsafeUnmount(callback){this.stage='unmounted';delete this.funicular.carriages[this.name][this.id];this.unsubscribe();if(typeof this.unmountedCallback=='function'){this.unmountedCallback(this)}if(typeof callback=='function'){callback()}}/**
   ...
   */},{key:'mountChilds',value:function mountChilds(callback){var _this3=this;var childCarriages=[];(0,_each2.default)(this.childs,function(child,nextChild){_this3.funicular.mount(child,function(error,carriage){if(error)nextChild(error);else{childCarriages.push(carriage);nextChild()}})},function(error){if(error){(0,_each2.default)(childCarriages,function(child,nextChild){child.unmount(function(){return nextChild()})},function(){callback(error)})}else{var _iteratorNormalCompletion=true;var _didIteratorError=false;var _iteratorError=undefined;try{for(var _iterator=childCarriages[Symbol.iterator](),_step;!(_iteratorNormalCompletion=(_step=_iterator.next()).done);_iteratorNormalCompletion=true){var child=_step.value;_this3.tieChild(child)}}catch(err){_didIteratorError=true;_iteratorError=err}finally{try{if(!_iteratorNormalCompletion&&_iterator.return){_iterator.return()}}finally{if(_didIteratorError){throw _iteratorError}}}callback()}})}/**
   ...
   */},{key:'moveRoots',value:function moveRoots(otherCarriage){otherCarriage.roots=this.roots;this.roots=[];for(var root in this.roots){this.funicular.roots[root]=otherCarriage}}}]);return Carriage}();/**
 * @example
...
this.roots={}}/**
   ...
   */_createClass(Funicular,[{key:'mount',value:function mount(name,mountedCallback,unmountedCallback){var carriage=new this.Carriage(name);var mountedCallback=mountedCallback||function(error){throw error};carriage.unmountedCallback=unmountedCallback;carriage.mount(function(error){mountedCallback(error,carriage)})}/**
   ...
   */},{key:'mountRoot',value:function mountRoot(name,mountedCallback,unmountedCallback){var _this6=this;var rootId=++this.lastRootId;this.mount(name,function(error,carriage){if(!error){carriage.tieRoot(rootId)}var unmountRoot=function unmountRoot(callback){if(_this6.roots[rootId]){var carriage=_this6.roots[rootId];carriage.untieRoot(rootId);carriage.unmount(callback)}else callback()};mountedCallback(error,carriage,unmountRoot)},unmountedCallback)}/**
